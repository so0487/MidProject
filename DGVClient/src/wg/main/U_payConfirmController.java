...
public class U_payConfirmController {
   ...
   public void setCou(U_payController cou) {
    ...
    private IBookMovieService bmService;
   ...
   @FXML
    void btnGoClick(ActionEvent event) throws IOException {
       ...
       // 성공 시 부모창 변경
       FXMLLoader loader = new FXMLLoader(U_payConfirmController.class.getResource("../fxml/U_payResult.fxml"));
       ...
       // 부모창 변경
       U_payResultController resultCtrl = loader.getController();
       resultCtrl.setInfoMap(infoMap);
       resultCtrl.setTopCtrl(topCtrl);
       resultCtrl.setSelectedSeats(selectedSeats);
       resultCtrl.setPage(infoMap);
       mainCtrl.setResult(result);
       ...
       try {
         ...
         bmService = (IBookMovieService) reg.lookup("bookMovieService");
         ...
         BookMovieVO bvo = new BookMovieVO();
         
         String book_id = bmService.getMaxBook_id();
         bvo.setBook_id(book_id);
         bvo.setBook_qty(Integer.parseInt(infoMap.get("book_qty")));
         bvo.setMem_id(LoginVO.getCurrVo().getMem_id());
         bvo.setShow_no(Integer.parseInt(infoMap.get("show_no")));
         
         // BookMovie테이블 insert
         cnt = bmService.insertBookMovie(bvo);
         if(cnt>0) {
            selectedSseats = mainCtrl.getSelectedSseats();
            
            for (int i = 0; i < selectedSseats.size(); i++) {
               Book_DetailVO bdvo = new Book_DetailVO();
               bdvo.setBook_id(book_id);
               int sseat_no = selectedSseats.get(i);
               bdvo.setSseat_no(sseat_no);
               // Book_detail테이블 insert
               bdService.insertBookDet(bdvo);
               
               // seatSch테이블 update
               ssService.updateToY(sseat_no);
            }
            
            PayVO pvo = new PayVO();
            pvo.setBook_id(book_id);
            int final_price = Integer.parseInt(infoMap.get("final_price"));
            pvo.setPay_price(final_price);
            
            int paymet_no = Integer.parseInt(infoMap.get("paymet_no"));
            pvo.setPaymet_no(paymet_no);
            // Pay테이블 insert
            pService.payBookMovie(pvo);
            
            if(infoMap.get("issueNo")!=null) {
               service.updateCou(Integer.parseInt(infoMap.get("issueNo")));
            }
            
            AlertUtil.infoMsg("예매성공", "예매가 완료되었습니다. 예매내역은 이메일로도 확인하실 수 있습니다.");
            makePDF(infoMap);
            sendEmail();
            
         }else {
            AlertUtil.warnMsg("작업결과", "잘못됐습니다.");
         }
    ...
    public void makePDF(Map<String, String> infoMap) {
       ...
       try {
          PdfWriter.getInstance(doc, new FileOutputStream("confirmation_letter.pdf"));
       ...
       try {
         ...
         String moviePos = infoMap.get("moviePos");
          ...
          PdfPCell theater = new PdfPCell(new Phrase("상영관 : "+theater_name));
          theater.setBorder(0);
          ...
          ///////////////////////////////////////////////////
            PdfPTable pay_table = new PdfPTable(1);
            PdfPCell pay_title;
            pay_title = new PdfPCell(new Phrase(""));
            pay_title.setBorder(1);
            pay_title.setBorderColor(BaseColor.GRAY);
            pay_title.setExtraParagraphSpace(15);
            pay_table.addCell(pay_title);
            doc.add(pay_title);
            doc.add(pay_table);
            ///////////////////////////////////////////////////
    ...
    public void sendEmail(){
        ...
        File file = new File("c:/soo//A_TeachingMaterial/4.MiddleProject/workspace/DGVClient/confirmation_letter.pdf"); //첨부 할 파일 경로
